...
const Enemies = (() => {
  ...
  function _createSmallGhost(col, row) {
    return { type:'ghost', x:col*TILE_SIZE_E, y:row*TILE_SIZE_E, w:34, h:40,
      ...
      floatPhase:Math.random()*Math.PI*2, opacity:0.75, alphaDir:1 };
  }
  ...
  function _updateSmallGhost(e, dt, map, ps) {
    ...
    if ((e.frozenTimer||0)>0){e.frozenTimer-=dt;e.vx*=0.85;e.vy*=0.85;}
      ...
      if (d<280&&d>8){e.vx+=(dx/d*65-e.vx)*3*dt;e.vy+=(dy/d*65-e.vy)*2.5*dt;}
      else{e.vx+=(Math.cos(e.floatPhase)*55-e.vx)*2*dt;e.vy+=(Math.sin(e.floatPhase*0.7)*30-e.vy)*2*dt;}
    }
